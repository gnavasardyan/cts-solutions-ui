...
- **Статус**: Частично решена - основной функционал работает корректно
- **Рабочие роли**: administrator, customer_operator (полностью функциональны)
- **Проблемные роли**: factory_operator, warehouse_keeper, site_master, auditor
- **Техническая проблема**: Разные экземпляры SQLite базы данных
- **Временное решение**: Система работает с двумя основными ролями
